...
def get_view(
    ...
    pose_i = CameraPose(transform=extrinsics[i])
    imaging_i = RaycastingImaging(resolution_image=images[0].shape,
                                  resolution_3d=intrinsics_dict[0]['resolution_3d'])
    points_i = pose_i.camera_to_world(points=imaging_i.image_to_points(image=image_i))
...
def pairwise_interpolate_predictions(
    ...
    reprojected_j = pose_i.world_to_camera(points=points_j)
    ...
    kdtree = cKDTree(uv_i)
    _, nn_indexes_in_i = kdtree.query(reprojected_j[:, :2], nn_set_size)
    ...
    for idx, point_from_j in tqdm(enumerate(reprojected_j)):
        ...
        if interp_mask[idx]:
            ...
            try:
                ...
                # original solution with bilinear interpolation
